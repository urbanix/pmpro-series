<?php
/*
Plugin Name: PMPro Series Multi
Plugin URI: http://www.paidmembershipspro.com/pmpro-series/
Description: Offer serialized (drip feed) content to your PMPro members, modified for different start dates for different series
Version: .1
...
/*
	Includes
*/
require_once(dirname(__FILE__) . "/classes/class.pmproseriesmulti.php");
...
function pmpros_the_content($content)
{
	...
	if($post->post_type == "pmpro_series" && pmpro_has_membership_access())
	{
		...
		$content .= "<p>You are on day " . intval(series_multi_pmpro_getMemberDays()) . " of your membership.</p>";
	...
	if(!empty($current_user) && !empty($post))
	{
		if(!pmpros_hasAccess($current_user->ID, $post->ID))
		{
			...
			if($inseries)
			{
				...
				$member_days = series_multi_pmpro_getMemberDays($current_user->ID);
...
//{
	...
	*/
	function series_multi_pmpro_getMemberStartdate($user_id = NULL, $level_id = 0)
	{
		if(empty($user_id))
		{
			global $current_user;
		...
		if(!empty($user_id) && (empty($level_id) || $level_id < 1))
		{
      ...
      $level_id = $current_membership->id;
		...
		if(empty($pmpro_startdates[$user_id][$level_id]))
		{
			...
			{
				$startdate = get_user_meta($user_id, 'pmpro_series_multi_startdate_' . $level_id, true);
			}
	...
	function series_multi_pmpro_getMemberDays($user_id = NULL, $level_id = 0)
	{
		if(empty($user_id))
		{
			global $current_user;
...
//
//
// NEW ADDITIONS TO PLUGIN
//
//


// Add composite fields to the "Advanced Settings" section of PMPro
function series_multi_advanced_settings() {
    $custom_fields = [];
		...
		{
			// Add a field for each level

			$custom_fields['field' . $field_number] = array(
					'field_name' => 'pmpro_series_multi_level_' . $level->id . '_composite' ,
					'field_type' => 'text',
					'label' => $level->name . ' ('. $level->id .') Composite Series',
					'description' => 'If this is a composite series, list the available series like \'1, 2, 4\'. Otherwise leave blank.'
			);
...
add_filter('pmpro_custom_advanced_settings', 'series_multi_advanced_settings');

// Add start dates for multiple series in user profile pages in admin panel
function series_multi_extra_user_profile_fields( $user ) {
		// Get the program levels and prices
		$pmpro_levels = pmpro_getAllLevels(false, true);


    ?>
    <h3><?php _e("Series Start Dates", "blank"); ?></h3>
    <table class="form-table">
			<?php
	    foreach($pmpro_levels as $level) {
				$level_id = $level->id;
				$composite_series = pmpro_getOption('pmpro_series_multi_level_' . $level->id . '_composite');

				if(empty($composite_series))
				{
					$start_date = get_user_meta($user->id, 'pmpro_series_multi_startdate_' . $level_id, true);
					// Add start date field if level is not a composite of multiple series
			?>
        <tr>
            <th><label for="pmpro_series_multi_startdate_<? echo $level_id; ?>"><?php _e($level->name); ?> <?php _e("Start Date"); ?></label></th>
            <td>
                <input type="text" name="pmpro_series_multi_startdate_<? echo $level_id; ?>" id="pmpro_series_multi_startdate_<? echo $level_id; ?>" class="regular-text"
                       value="<?php echo esc_attr( $start_date ); ?>" /><br />
                <span class="description"><?php _e("Modify this user's start date for the $level->name series. Format: 2010-01-01 12:00:00"); ?></span>
            </td>
        </tr>
			<?php
				}
...
add_action( 'edit_user_profile', 'series_multi_extra_user_profile_fields' );

// Save start dates for multiple series when user profiles are edited by admins
function series_multi_save_extra_user_profile_fields( $user_id ) {
    global $wpdb;
    ...
    if ( current_user_can( 'edit_user', $user_id ) ) {
			...
			foreach($pmpro_levels as $level) {
				...
				if(empty($composite_series))
				{
					// Save start date if the level is not part of a composite series

					// The date format is Y-m-d H:i:s
					update_user_meta($user_id, 'pmpro_series_multi_startdate_' . $level->id, $_POST['pmpro_series_multi_startdate_' . $level->id]);
	        $saved = true;
				}
			}
    }
...
add_action( 'edit_user_profile_update', 'series_multi_save_extra_user_profile_fields' );

// Set series start dates on checkout
function series_multi_pmpro_checkout_start_date($startdate, $user_id, $pmpro_level)
{
	$level = $pmpro_level;
	$composite_series = pmpro_getOption('pmpro_series_multi_level_' . $level->id . '_composite');
	if(empty($composite_series))
	{
		// If it's not a composite series, simply set the start date as the current date
		update_user_meta($user_id, 'pmpro_series_multi_startdate_' . $level->id, $startdate);
	}
	else
	{
		// If it is a composite level
		$levels_to_update = explode(',', $composite_series);

		foreach($levels_to_update as $level_to_update)
		{
			// Loop through the related series

			$old_startdate = get_user_meta($user_id, 'pmpro_series_multi_startdate_' . trim($level_to_update), true);

			// Update if there isn't an existing start date
			if(empty($old_startdate))
				update_user_meta($user_id, 'pmpro_series_multi_startdate_' . trim($level_to_update), $startdate);
		}
	}

	return $startdate;
}
add_filter('pmpro_checkout_start_date', 'series_multi_pmpro_checkout_start_date', 99, 3);
